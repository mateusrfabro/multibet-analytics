...
Fluxo:
  ...
  5. Validacao cruzada BigQuery vs Athena
  6. Gera CSV + ZIP com TODOS os opt-in (inativos com "Sem Atividade")
"""
...
# ---------------------------------------------------------------------------
MARK_TAG = "GIRE_GANHE_SUGAR_RUSH_260327"

# Sugar Rush (Pragmatic Play) -- 2 variantes no catalogo Athena
...
# Sugar Rush no catalogo Smartico (BigQuery) -- para validacao cruzada
SMR_GAME_ID = 45928724

# Periodo: 26/03 19h BRT = 26/03 22:00 UTC | 26/03 23h59 BRT = 27/03 02:59 UTC
START_UTC = "2026-03-26 22:00:00"
END_UTC   = "2026-03-27 02:59:59"
...
FAIXAS = [
    ("Faixa 4", 500.00, float("inf")),   # >= R$500
    ("Faixa 3", 300.00, 499.99),          # R$300 -- R$499,99
    ("Faixa 2", 100.00, 299.99),          # R$100 -- R$299,99
    ("Faixa 1",  30.00,  99.99),          # R$30 -- R$99,99
]

OUTPUT_CSV = os.path.join(PROJECT_ROOT, "segmentacao_promo_relampago_260326_FINAL.csv")
OUTPUT_ZIP = os.path.join(PROJECT_ROOT, "segmentacao_promo_relampago_260326_FINAL.zip")
...
def build_sql(chunk_ids: list) -> str:
    ...
    - c_product_id = 'CASINO': filtro obrigatorio (fund_ec2 mistura casino+sports)
    """
...
def main():
    ...
    rotulos = {
        ...
        "Abaixo do Minimo": "Abaixo de R$30",
    ...
    SEP = "=" * 70
    print(f"\n{SEP}")
    print(f"  Segmentacao Promo Relampago | Sugar Rush | 26/03/2026")
    print(f"  Promocao: {MARK_TAG}")
    print(f"  Periodo : 26/03 19h -> 23h59 BRT (UTC: {START_UTC} -> {END_UTC})")
    print(SEP)

    print(
        f"\nDo segmento com opt-in ({total_marcados:,} usuarios marcados), "
        f"{athena_jogadores:,} jogaram Sugar Rush no periodo. "
    ...
    for faixa in ordem_faixas:
        ...
        if faixa in ("Sem Atividade", "Abaixo do Minimo"):
            ...
            print(f"  * {label}: {n:,} jogador(es)")
        else:
            print(f"  * {label}: {n:,} jogador(es) -- {fmt_brl(vol)} ({pct:.0f}% do volume)")

    elegiveis = sum(faixa_counts.get(f, 0) for f in ["Faixa 1", "Faixa 2", "Faixa 3", "Faixa 4"])
    print(f"\nElegiveis para pagamento: {elegiveis:,} jogadores.")

    print("\nPonto de atencao:")
    if total_marcados > 0:
        print(
            f"  Apenas {athena_jogadores/total_marcados*100:.1f}% dos marcados "
            f"efetivamente jogaram no periodo."
        )
    if desclassificados == 0:
        print("  Zero rollbacks -- nenhum desclassificado.")
    else:
        print(
            f"  {desclassificados:,} desclassificado(s) por rollback."
        )

    # Destaque de concentracao
    for faixa in ["Faixa 4", "Faixa 3", "Faixa 2", "Faixa 1"]:
        n = faixa_counts.get(faixa, 0)
        vol = df_final[df_final["faixa_segmentacao"] == faixa]["total_bet_brl"].sum()
        pct = vol / total_apostado * 100 if total_apostado > 0 else 0
        if pct > 40:
            print(f"  {rotulos[faixa]} concentra {pct:.0f}% do volume com {n:,} jogador(es).")

    print("\n--- Validacoes realizadas ---\n")
    validacoes = [
        f"Jogo confirmado no catalogo Athena: Sugar Rush (vs20sugarrush + 204, Pragmatic Play).",
        f"Usuarios extraidos do BigQuery Smartico via tag '{MARK_TAG}' "
        f"em j_user.core_tags -- {total_marcados:,} com opt-in confirmado.",
        "Valores confirmados em centavos (c_amount_in_ecr_ccy, Pragmatic v1.3) -- divisao por 100 aplicada no SQL.",
        "Status c_txn_status = 'SUCCESS' validado empiricamente no schema fund_ec2.",
        f"Rollbacks (txn_type=72): {int(df_final['qtd_rollbacks'].sum()):,} ocorrencias "
        f"-- {'nenhum desclassificado' if desclassificados == 0 else str(desclassificados) + ' desclassificado(s)'}.",
        "Mapeamento de IDs validado: Smartico user_ext_id = c_external_id na tabela ECR.",
        "Cada jogador aparece em apenas uma faixa (a mais alta atingida) -- sem duplicidade de pagamento.",
        f"Periodo em UTC: {START_UTC} -> {END_UTC} (equivalente a 26/03 19h -- 23h59 BRT).",
        "Dados extraidos do Athena (Iceberg Data Lake) -- banco operacional principal do projeto.",
        "Turnover = soma acumulada no Sugar Rush (Wallet Share do evento).",
        f"Duplo check BigQuery vs Athena: {athena_jogadores} vs "
        f"{bq_result['qtd_jogadores']} jogadores, diferenca de {fmt_brl(diff_valor)} "
        f"({pct_diff:.2f}%) -- {'dados consistentes' if pct_diff < 5 else 'ATENCAO: divergencia > 5%'}.",
        f"CSV inclui TODOS os {total_marcados:,} com atividade -- elegiveis e nao elegiveis separados.",
    ]
