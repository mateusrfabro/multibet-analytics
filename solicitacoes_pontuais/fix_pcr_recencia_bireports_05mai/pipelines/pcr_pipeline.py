...
Fonte:  bireports_ec2.tbl_ecr_wise_daily_bi_summary + ps_bi.dim_user (Athena)
        (era ps_bi.fct_player_activity_daily ate 2026-05-05; migrado por gap dbt)
...
def extrair_metricas_jogadores() -> pd.DataFrame:
    ...
    -- Fonte: bireports_ec2.tbl_ecr_wise_daily_bi_summary + dim_user
    -- Valores em centavos no banco — divididos por 100 aqui pra retornar BRL.
    --
    -- HISTORICO 2026-05-05 (Mateus): migrado de ps_bi.fct_player_activity_daily
    -- para bireports_ec2 porque a fato dbt parou de materializar em 06/04 e
    -- inflou recency_days + congelou agregados financeiros. Bateu auditoria
    -- empirica top 50 jan-mar/2026: GGR delta global -0.68%, depositos -0.40%,
    -- mediano 0%. Bireports e raw atualizada D-1, ja consumida por outras
    -- views/silvers do squad (ec2 sync diario).
    --
    -- DEFINICAO DE ATIVO (mantida de v1.1):
    ...
    WITH player_metrics AS (
        SELECT
            ...
            ), 0) / 100.0 AS ggr_total,
            -- NGR aproximado: GGR menos bonus issued (bireports nao tem bonus_turned_real
            -- direto. bonus_issued e mais conservador — penaliza um pouco mais).
            COALESCE(SUM(
                (s.c_casino_realcash_bet_amount - s.c_casino_realcash_win_amount)
              + (s.c_sb_realcash_bet_amount     - s.c_sb_realcash_win_amount    )
              -  s.c_bonus_issued_amount
            ), 0) / 100.0 AS ngr_total,
            ...
            COALESCE(SUM(s.c_casino_realcash_bet_count), 0)          AS casino_rounds,

            -- Sportsbook
            COALESCE(SUM(s.c_sb_realcash_bet_amount), 0) / 100.0 AS sport_bet,
            COALESCE(SUM(s.c_sb_realcash_win_amount), 0) / 100.0 AS sport_win,
            COALESCE(SUM(s.c_sb_realcash_bet_count), 0)          AS sport_bets,

            -- Depositos
            COALESCE(SUM(s.c_deposit_success_count), 0)           AS num_deposits,
            COALESCE(SUM(s.c_deposit_success_amount), 0) / 100.0  AS total_deposits,

            -- Saques (cashout)
            COALESCE(SUM(s.c_co_success_count), 0)           AS num_cashouts,
            COALESCE(SUM(s.c_co_success_amount), 0) / 100.0  AS total_cashouts,

            -- Atividade
            COUNT(DISTINCT s.c_created_date) AS days_active,
            MAX(s.c_created_date) AS last_active_date,
            MIN(s.c_created_date) AS first_active_date,

            -- Bonus
            COALESCE(SUM(s.c_bonus_issued_amount), 0) / 100.0 AS bonus_issued,
            -- bonus_turned_real nao existe direto na bireports — mantem 0.
            -- Quando dbt voltar, considerar reintroduzir via fct_player_activity_daily
            -- ou criar campo dedicado em silver.
            CAST(0 AS DOUBLE) AS bonus_turned_real,

            -- Turnover total (casino + sport)
            COALESCE(SUM(
                s.c_casino_realcash_bet_amount + s.c_sb_realcash_bet_amount
            ), 0) / 100.0 AS turnover_total

        FROM bireports_ec2.tbl_ecr_wise_daily_bi_summary s
        WHERE s.c_created_date >= CURRENT_DATE - INTERVAL '{JANELA_DIAS}' DAY
          AND s.c_created_date < CURRENT_DATE  -- exclui D-0 (parcial)
        GROUP BY s.c_ecr_id
        -- FILTRO CRITICO: somente quem apostou ou depositou
        HAVING COALESCE(SUM(s.c_casino_realcash_bet_count), 0) > 0
            OR COALESCE(SUM(s.c_sb_realcash_bet_count), 0) > 0
            OR COALESCE(SUM(s.c_deposit_success_count), 0) > 0
    )
    SELECT
        m.*,
        u.external_id,
