...
Output em reports/afiliados_marco_auditoria/:
  - afiliados_consolidado.csv          (12 IDs unicos)
  - sports_marco_afiliados.csv         + _legenda.txt
  - casino_marco_afiliados.csv         + _legenda.txt
  - geral_marco_afiliados.csv          + _legenda.txt
  - README_auditoria.md
...
OUT_DIR = os.path.join(r"c:\Users\NITRO\OneDrive - PGX\Projetos - Super Nova\MultiBet", "reports", "afiliados_marco_auditoria")
...
# LEGENDAS / DICIONARIO
# =====================================================================
LEGENDA_SPORTS = f"""LEGENDA — sports_marco_afiliados.csv
...
- ps_bi.dim_user.affiliate_id IN (12 afiliados alvo)
...
LEGENDA_CASINO = f"""LEGENDA — casino_marco_afiliados.csv
Periodo: {PERIOD_LABEL}
...
LEGENDA_GERAL = f"""LEGENDA — geral_marco_afiliados.csv
Periodo: {PERIOD_LABEL}
...
2. `sports_marco_afiliados.csv` + `_legenda.txt` — apostas esportivas (vendor_ec2)
3. `casino_marco_afiliados.csv` + `_legenda.txt` — transacoes de cassino (fund_ec2 CASINO)
4. `geral_marco_afiliados.csv`  + `_legenda.txt` — todas transacoes financeiras (fund_ec2)
...
## Lista de afiliados (12 IDs unicos)
...
def main():
    ...
    # Salvar legendas
    with open(os.path.join(OUT_DIR, "sports_marco_afiliados_legenda.txt"), "w", encoding="utf-8") as f:
        f.write(LEGENDA_SPORTS)
    with open(os.path.join(OUT_DIR, "casino_marco_afiliados_legenda.txt"), "w", encoding="utf-8") as f:
        f.write(LEGENDA_CASINO)
    with open(os.path.join(OUT_DIR, "geral_marco_afiliados_legenda.txt"), "w", encoding="utf-8") as f:
